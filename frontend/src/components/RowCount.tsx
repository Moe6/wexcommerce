...
const RowCount: React.FC<RowCountProps> = ({
  ...
  return language && (
    <span className={styles.rowCount}>
      {`${(page - 1) * pageSize + 1}-${rowCount} ${commonStrings.OF_OVER} ${lebobeautycoHelper.formatNumber(totalRecords, language)} ${totalRecords === 1 ? commonStrings.RESULT : commonStrings.RESULTS}`}
    </span>
  )
}
